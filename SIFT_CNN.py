...
def convnet(X, Y, SIFT, convlayer_sizes=[10, 10], filter_shape=[3, 3], outputsize=10, padding="same"):
    """
    ...
    denseSIFT = tf.layers.dense(SIFT,num_features,activation=tf.nn.leaky_relu)
...
def train(sess, data, n_epochs, batch_size,
    ...
    train_data.append(SIFT_data_train)
    test_data.append(SIFT_data_test)
...
if __name__ == "__main__":
	...
	for pixel_sequence in train_dt['pixels']:
	    ...
	    raw_faces.append(np.array(face, dtype=np.uint8))
	...
	if N_CLUSTER > batch_size:
		N_CLUSTER = int(sqrt(batch_size))
